#!/usr/bin/env python3
"""
spinny-scraper — 2captcha Scraper API edition (fourth engine)
=================================================================
...
--------------------------------------
Spinny is the WORST fit in this family for a browserless path, and this
client is here for completeness rather than as a recommendation. Expect it
to return **no cars at all**, and that is the client working correctly.

The reason is structural rather than fixable here, and it is the same fact
the browser engines are built around: **Spinny's first response is a shell.**
Measured 2026-09-11 — `domcontentloaded` on /used-cars-in-delhi-ncr/s/ at
1.1 s with ZERO cars in the DOM, the first batch of 22 arriving at 4.7 s from
an api.spinny.com XHR, and the full 302 only after ten scroll rounds. A
single HTTP fetch captures the shell: 1.6 to 2 MB of navigation, filter rail
and footer, with the grid container present and empty.

So this path cannot see a listing at all, by construction — not a fraction of
one, none of it. Measured 2026-09-11, `tasks/sync` routed through a Scraping
Browser session against /used-cars-in-delhi-ncr/s/:

    upstream status   200
    HTML              1,873,849 bytes
    cars parsed       0
    cost              $0.0005
    exit code         4

Against 62 from the same URL through `playwright_scraper.py`. A sibling site
in this family server-renders about five tiles into that first response,
which at least demonstrates the path; here there is nothing to demonstrate it
with.

It is kept for three reasons. It costs nothing to keep. `--cdp-url` routes it
through a Scraping Browser session, so the paid path is exercised and stays
honest. And a browserless fetch is genuinely the right tool on other sites in
this family, where the grid is server-rendered — this client is family core,
and family core that only exists in four repos and runs in three is how the
untested-inheritance defects in §16 happen.

If you are reading this because your run came back with zero rows: use
`playwright_scraper.py`, which scrolls.
...
-----
    ...
    # $SPINNY_CDP_ENDPOINT, so neither needs to be typed — a secret in
...
def _run_once(args, attempt: int = 1, attempts: int = 1) -> int:
    ...
    if state == "blocked":
        ...
        with open(dump, "w", encoding="utf-8") as f:
            f.write(html)
        logger.error(
            "This response was not built by Spinny (upstream HTTP %s, %d "
            "bytes) — saved to %s. No Spinny refusal has ever been observed, "
            "from a datacentre exit or a residential one, so suspect the "
            "path rather than the site: an unreachable --cdp-url, or a "
            "Scraping Browser profile another run still holds. This is exit "
            "3, distinct from an empty result (exit 4) — and note that on "
            "this site a SUCCESSFUL fetch also returns no cars, because the "
            "grid arrives by XHR after load. See this file's docstring.",
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Spinny scraper — 2captcha Scraper API edition (no local "
                    "browser). NOTE: a Spinny listing's first response is a "
                    "SHELL — the grid arrives by XHR after load and grows "
                    "only as the page is scrolled — so a single fetch "
                    "returns NO cars however it is routed. Kept for family "
                    "parity; see this file's docstring, and use "
                    "playwright_scraper.py, which scrolls.")
    ...
    p.add_argument("--url", default=None,
                   help="Spinny listing URL, e.g. "
                        "/used-cars-in-delhi-ncr/s/. Required, unless "
                        "SPINNY_URL is set in the environment or in .env. "
                        "Note that no listing URL yields cars through this "
                        "path — the grid is not in the first response.")
    ...
    p.add_argument("--out", default="spinny_products_scraperapi", help="Output file prefix")
