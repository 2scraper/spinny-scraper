...
# Contexts in which a 32-hex string is plainly not a key. The last one is
# this site's: every Spinny car photograph is served from its asset host
# under a 32-hex object id —
#
#     https://assets.spinny.com/sp-file-system/public/<date>/<32 hex>/raw/…
#
# so the committed fixtures carry several, and without this entry the repo's
# own credential grep fails on its own test data. Kept as a CONTEXT
# allowlist rather than by loosening the pattern: a bare 32-hex string
# anywhere else still fails, which is the point of the check.
HEX32_ALLOWED = ("sha", "hash", "nonce", "example", "md5", "digest",
                 "checksum", "assets.spinny.com")
