...
page. If Spinny renders a reCAPTCHA challenge anywhere — account, sign-in
and the sell-your-car flow are the usual places — this fires.

**NO RENDERED challenge has been observed on this site, and that is not the
same as no captcha being configured.** Spinny wires reCAPTCHA **Enterprise
v3** into every page it serves — 31 occurrences of "recaptcha" on a capture
that had just delivered 482 cars, an invisible widget with a badge the
site's own CSS hides — and v3 SCORES a session rather than challenging it.
So the machinery is configured everywhere and fires nowhere: there is
nothing rendered for a visitor to solve and nothing for a solver to buy
unless the site starts gating the grid on a score, which it has not been
observed doing from either exit tested (measured 2026-09-11).

This module is therefore a contingency rather than part of the happy path —
a bot manager can be switched on between deploys, and a scraper that cannot
name what stopped it is much harder to fix. What it WOULD buy if one
appeared is an enterprise v3 score task, which `product_parser.recaptcha_config`
reads off the site's own loader so the wrong task type is not purchased.
...
# There was a `get_balance()` here, for "a preflight check", and it is gone
# with the endpoint constant it used. Nothing called it: this module has no
# CLI, no engine consulted it before spending, and adding a paid API call to
# every run to find out whether a run might spend anything would cost more
# than it saved. A public helper with no consumer is the defect §17 names —
# it reads like a feature and is not one — so it is removed rather than left
# to look load-bearing. If a balance check is ever wanted, it belongs behind
...
# of it is ported here, because Spinny has no such page.
#
# What Spinny does instead is serve the page. Measured 2026-09-11 from an
# Amsterdam datacentre exit and a Chennai residential exit: both got the full
# grid, with no key and no proxy, and no refusal of any kind was observed
# from either. The reCAPTCHA v3 the site does load scores the session in the
# background and rendered nothing to solve on any capture. So the state a
# blocked run here is most likely to have met is not the site at all but a
# dead proxy exit or Chromium's own error page, and
# product_parser.detect_page_state reports that as "blocked" rather than
# "challenge" precisely so no solve is attempted or billed.
#
