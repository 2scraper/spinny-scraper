...
======================================================
Dual mode: direct API (fast, no browser) + browser fallback with selenium-wire.
...
def scrape_api_sync(cats, max_pages, proxy=None):
    ...
    s = req.Session()
    s.headers.update({"User-Agent": USER_AGENTS[0], "Referer": f"{BASE_URL}/used-cars/s/", "Accept": "application/json"})
    ...
    for cat in cats:
        ...
        for pg in range(1, max_pages + 1):
            ...
            try:
                r = s.get(API_BASE, params=params, timeout=20); r.raise_for_status(); body = r.json()
...
# ---------------------------------------------------------------------------
# MODE 2: Browser with selenium-wire interception
# ---------------------------------------------------------------------------

def scrape_browser(cats, args):
    opts = Options()
            ...
            for pg in range(1, args.pages + 1):
                ...
                if HAS_WIRE:
                    for r in drv.requests:
                        if r.response and r.response.status_code == 200 and "listing" in r.url and "api.spinny.com" in r.url:
                            try:
                                b = json.loads(r.response.body.decode("utf-8"))
                                res, _, _ = extract_results(b)
                                found.extend(parse_spinny_results(res, cat))
                            except Exception: pass
                if found:
                    results.extend(found); log.info("  → %d listings", len(found))
                else:
...
def main():
    ...
    if args.mode in ("api", "auto"):
        ...
        try:
            ...
            log.warning("requests not installed (pip install requests). Falling back to browser.")
    ...
    if args.mode == "browser" or (args.mode == "auto" and not results):
        results = scrape_browser(cats, args)
