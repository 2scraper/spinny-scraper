...
# What a price monitor watching Spinny actually wants to be told about.
#
# `price_all_in` is tracked ALONGSIDE `price`, and that pair is the point on
# this site: the displayed figure excludes RC transfer and insurance while
# the site's own `data-price` attribute includes them, so the two can move
# independently. A car whose displayed price is unchanged while its all-in
# figure rose has had a fee changed, not a price cut, and a monitor that
# watched only one of them would report the wrong thing in both directions.
#
# `discount_amount` is the site's own exact rupee figure and is tracked as a
# CROSS-CHECK on `discount_pct`, which this repo computes: the two moving
# apart means the read has broken rather than the price having changed.
#
# `rating` and `review_count` are NOT here, and their absence is measured
# rather than an omission — Spinny publishes neither anywhere, so tracking
# them would add two columns that can never differ. `sold` / `sold_is_floor`
# are a sibling repo's and do not exist here; nor do `price_is_from` /
# `price_max`, because a Spinny card prints one price, not a range.
TRACKED_FIELDS = ("price", "original_price", "discount_pct", "currency",
                  "in_stock", "price_all_in", "discount_amount")
...
def _within_tolerance(before: dict, after: dict, changes: dict,
    ...
    SPINNY BEHAVIOUR WAS MEASURED — the site sells in one country and quotes
    INR to every visitor, verified identical from an Amsterdam datacentre
    exit and a Chennai residential one, so a run has no conversion in it and
    every rupee of a difference is a real price move.
...
def _check_comparable(args) -> bool:
    ...
    # hostname alone. Spinny is ONE host with ONE currency — measured
    # 2026-09-11, an Amsterdam datacentre exit and a Chennai residential exit
    # returned the same page, the same advertised total and INR prices both
    # times — so `source` is "spinny.com" on both sides and there is no
    # storefront split for it to catch.
    #
    ...
    if len(set(currencies.values())) > 1:
        problems.append(
            ...
            f"Spinny quotes INR to every visitor — verified identical from "
            ...
            f"`source` cannot catch it: it is 'spinny.com' on both sides.")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Diff two spinny-scraper JSON outputs by sku.")
    ...
    p.add_argument("--price-tolerance-pct", type=float, default=0.0,
                   ...
                   help="Treat a price move smaller than PCT%% as an exchange-"
                        ...
                        "(report every rupee), which is what a Spinny "
                        "run wants: the site quotes INR to every visitor, so "
